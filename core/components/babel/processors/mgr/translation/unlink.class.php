...
class unlinkBabelTranslation extends BabelProcessor {
    ...
    public function unlink() {
        ...
        if(!isset($this->linkedResources[$this->contextKey])) {
            ...
            return $this->modx->error->failure('No link for this context');
        }
        ...
        if($this->linkedResources[$this->contextKey] == $this->resource->get('id')) {
            ...
            return $this->modx->error->failure('Self unlink is not possible');
        }
        ...
        if(!$unlinkedResource) {
            ...
            return $this->modx->error->failure('No link found for that context');
        }
...
//    throw new Exception('error.resource_from_other_context');
            return $this->modx->error->failure('Resource is from another context');
        }
