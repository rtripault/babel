...
switch ($modx->event->name) {
    case 'OnDocFormPrerender':
        ...
        //$modx->log(modX::LOG_LEVEL_ERROR, $outputLanguageItems);
        ...
        $modx->regClientStartupScript('<script type="text/javascript">
            Ext.onReady(function() {
                //var Babelconfig = { translations: '. $modx->toJSON($list) .' };
                Babel.config = '. $modx->toJSON($babel->config) .';
                Babel.config.translations = '. $modx->toJSON($list) .';

                var modAB = Ext.getCmp("modx-action-buttons");
                if (modAB) {
                    //var translations = new Babel.Translations(config = Babelconfig);
                    var translations = new Babel.Translations();
                    // Keep the spacing between buttons
                    var split = "-";

                    modAB.insert(0, translations);
                    modAB.insert(1, split);
                    modAB.doLayout();
                }
            });
        </script>', 1);
...
//
//            Ext.onReady(function() {
//                //var babel.config = "'. $modx->toJSON($outputLanguageItems).'";
//
//                var modAB = Ext.getCmp("modx-action-buttons");
//                if (modAB) {
//                    var translations = new Ext.SplitButton({
//                        text: "Translations"
